...
import com.teamsorcerers.librarianlib.core.util.kotlin.getOrNull
import com.teamsorcerers.librarianlib.core.util.kotlin.inconceivable
...
internal object IdentifierSerializer: NbtSerializer<Identifier>() {
    override fun deserialize(tag: NbtElement): Identifier {
        return Identifier.of(tag.expectType<NbtString>("tag").asString())
    }
...
internal object BlockPosSerializer: NbtSerializer<BlockPos>() {
    override fun deserialize(tag: NbtElement): BlockPos {
        return NbtHelper.toBlockPos(tag.expectType("tag"), "tag").get()
    }
...
internal class TextSerializerFactory(prism: NbtPrism): NBTSerializerFactory(prism, Mirror.reflect<Text>()) {
    ...
    class TextSerializer(type: ClassMirror): NbtSerializer<Text>(type) {
        ...
        companion object {
            private val LOOKUP: WrapperLookup = BuiltinRegistries.createWrapperLookup();
        }

        override fun deserialize(tag: NbtElement): Text {
            val component = fromJson(tag.expectType<NbtString>("tag").asString(), LOOKUP)
        ...
        override fun serialize(value: Text): NbtElement {
            return NbtString.of(
                TextCodecs.CODEC.encodeStart<JsonElement>(LOOKUP.getOps<JsonElement>(JsonOps.INSTANCE), value)
                    .getOrThrow<JsonParseException>(
                        Function<String, JsonParseException> { msg: String? ->
                            JsonParseException(
                                msg
                            )
                        }).toString()
...
internal object BlockStateSerializer: NbtSerializer<BlockState>() {
    override fun deserialize(tag: NbtElement): BlockState {
        return BlockState.CODEC.parse(NbtOps.INSTANCE, tag.expectType("tag")).result().orElse(null)
    }
...
internal object ItemStackSerializer: NbtSerializer<ItemStack>() {
    override fun deserialize(tag: NbtElement): ItemStack {
        return ItemStack.CODEC.parse(NbtOps.INSTANCE, tag.expectType("tag")).result().orElse(null)
    }

    override fun serialize(value: ItemStack): NbtElement {
        return ItemStack.CODEC.encodeStart(NbtOps.INSTANCE, value).result().orElse(null)
    }
}
...
internal object EnchantmentLevelEntrySerializer: NbtSerializer<EnchantmentLevelEntry>() {
    override fun deserialize(tag: NbtElement): EnchantmentLevelEntry {
        ...
        @Suppress("NAME_SHADOWING") val tag = tag.expectType<NbtCompound>("tag")
        val id = Identifier.of(tag.expect<NbtString>("Enchantment").asString())
        val enchantment = enchantmentRegistry.getEntry(id).getOrNull()
    ...
    override fun serialize(value: EnchantmentLevelEntry): NbtElement {
        val rm = MinecraftClient.getInstance().world?.registryManager
            ?: throw Exception("Couldn't get world instance.");

        val enchantmentRegistry = rm.get(RegistryKeys.ENCHANTMENT);

        val tag = NbtCompound()
        tag.put("Enchantment", NbtString.of(enchantmentRegistry.getId(value.enchantment.value()).toString()))
